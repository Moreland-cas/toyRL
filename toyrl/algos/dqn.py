...
class DQN_test():
    ...
    def take_action(self, state):
        ...
        return action
def train_dqn(save_image=True, save_ckpt=True):
    lr = 2e-3
    num_episodes = 500
    hidden_dim = 128
    gamma = 0.98
    ...
    if save_image:
        ...
        # 可以添加标题和标签
        plt.title(f'Plot of DQN')
        plt.xlabel('episode')
        plt.ylabel('reward')

        # 保存图像
        plt.savefig(f'dqn.png')  # 保存为PNG格式的文件
    
    if save_ckpt:
        torch.save(agent.q_net.state_dict(), 'dqn_ckpt.pth')
    return return_list

def test_dqn(ckpt_path="dqn_ckpt.pth"):
    hidden_dim = 128
    ...
    state_dim = env.observation_space.shape[0]
    action_dim = env.action_space.n
    agent = DQN_test(state_dim, hidden_dim, action_dim, device, ckpt_path)
